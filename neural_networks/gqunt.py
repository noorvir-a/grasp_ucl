...
from nn_util import NeuralNets, NeuralNetLayers
...
class GQUNt(object):
    ...
    # def _create_network(self, input_im_data):
    def _create_network(self, input_im_data, input_pose_data, initialiser):
        """ Create network"""
        ...
        self.layers.conv1_1 = self.util.convolution(input_im_data, filter_dim=7, num_filters=64, pool_stride=1, name='conv1_1',
        ...
        self.layers.conv1_2 = self.util.convolution(input_im_data, filter_dim=5, num_filters=64, pool_stride=1, name='conv1_2',
                                                    initialiser=initialiser)
        ...
        self.layers.conv1_1 = self.util.convolution(input_im_data, filter_dim=3, num_filters=64, pool_stride=1, name='conv2_1',
                                                    initialiser=initialiser)
        ...
        self.layers.conv2_2 = self.util.convolution(input_im_data, filter_dim=3, num_filters=64, pool_stride=1, name='conv2_2',
                                                    initialiser=initialiser)
        ...
        self.layers.pc1 = self.util.fully_connected(input_pose_data, 1, 16, initialiser, name='pc1')
    ...
    def _get_prediction_network(self, initialiser=None):
        ...
        with self._graph.as_default():
            ...
            self._pred_pose_node = tf.placeholder(tf.float32, [self.nn.batch_size, self.nn.pose_size], name='prediction_pose_node')
            ...
            with tf.name_scope('prediction_network'):
                with tf.variable_scope(tf.get_variable_scope(), reuse=True):
                    self._pred_network_output = self._create_network(self._pred_input_node, self._pred_pose_node, initialiser)
    ...
    def optimise(self, weights_init='pre_trained'):
        ...
        if weights_init == 'pre_trained':
            ...
            if self.nn.weights_init_type == 'gaussian':
                ...
                initialiser = tf.contrib.xavier_initializer()

        self.nn.network_output = self._create_network(self.nn.train_input_node, self.nn.train_pose_node, initialiser)

        # init validation network
        self._get_prediction_network(initialiser)
            ...
            for epoch in xrange(1, self.nn.config['num_epochs'] + 1):
                ...
                for batch in xrange(1, batches_per_epoch + 1):
                    ...
                    if batch % self.nn.val_frequency == 0:
                        ...
                        logging.info(self.nn.get_date_time() + ': epoch = %d, batch = %d, validation accuracy = %.3f' % (
                        epoch, batch, val_accuracy))
                        logging.info('------------------------------------------------')
                    ...
                    if batch % self.nn.save_frequency == 0:
                        ...
                        logging.info('------------------------------------------------')
                        logging.info(self.nn.get_date_time() + ': epoch = %d, batch = %d, accuracy = %.3f' % (
                        epoch, batch, self.nn.train_accuracy))
