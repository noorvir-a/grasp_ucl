...
class SGDOptimizer(object):
    ...
    def _create_loss(self):
        ...
        if self.cfg['loss'] == 'l2':
            ...
            return tf.reduce_mean(
                tf.nn.sparse_softmax_cross_entropy_with_logits(_sentinel=None, labels=self.train_labels_node,
                                                               logits=self.train_net_output, name=None))
    ...
    def optimize(self):
        ...
        with tf.name_scope('training_network'):
            self.train_net_output = self.gqcnn._build_network(self.input_im_node, self.input_pose_node, drop_fc3,
        ...
        try:
            ...
            for step in training_range:
                ...
                _, l, lr, predictions, batch_labels, output, train_images, conv1_1W, conv1_1b, pose_node = self.sess.run(
                    [optimizer, loss, learning_rate, train_predictions, self.train_labels_node, self.train_net_output,
                     self.input_im_node, self.weights.conv1_1W, self.weights.conv1_1b, self.input_pose_node],
                ...
                if step % self.log_frequency == 0:
                    ...
                    self.summary_writer.add_summary(self.sess.run(self.merged_log_summaries, feed_dict={
                        self.minibatch_error_placeholder: train_error, self.minibatch_loss_placeholder: l,
                        self.learning_rate_placeholder: lr}), step)
                    sys.stdout.flush()

                    # update the TrainStatsLogger
                    self.train_stats_logger.update(train_eval_iter=step, train_loss=l, train_error=train_error,
                                                   total_train_error=None, val_eval_iter=None, val_error=None,
                                                   learning_rate=lr)
                ...
                if step % self.eval_frequency == 0:
                    if self.cfg['eval_total_train_error']:
                        ...
                        self.train_stats_logger.update(train_eval_iter=None, train_loss=None, train_error=None,
                                                       total_train_error=train_error, val_eval_iter=None,
                                                       val_error=None, learning_rate=None)
                        self.train_stats_logger.log()

                    val_error = self._error_rate_in_batches()
                    self.summary_writer.add_summary(
                        self.sess.run(self.merged_eval_summaries, feed_dict={self.val_error_placeholder: val_error}),
                        step)
                    ...
                    # update the TrainStatsLogger
                    self.train_stats_logger.update(train_eval_iter=None, train_loss=None, train_error=None,
                                                   total_train_error=None, val_eval_iter=step, val_error=val_error,
                                                   learning_rate=None)
    ...
    def _read_pose_data(self, pose_arr, input_data_mode):
        ...
        if input_data_mode == InputDataMode.TF_IMAGE:
            ...
            raise ValueError('Input data mode %s not supported. The RAW_* input data modes have been deprecated.' % (
            input_data_mode))
    ...
    def _setup_tensorflow(self):
        ...
        with tf.name_scope('train_data_node'):
            self.train_data_batch = tf.placeholder(tf.float32, (
            self.train_batch_size, self.im_height, self.im_width, self.num_tensor_channels))
        ...
        with tf.name_scope('data_queue'):
            self.q = tf.FIFOQueue(self.queue_capacity, [tf.float32, tf.float32, train_label_dtype], shapes=[
                (self.train_batch_size, self.im_height, self.im_width, self.num_tensor_channels),
    ...
    def _compute_data_metrics(self):
        ...
        if self.cfg['fine_tune']:
            ...
            for k in random_file_indices.tolist():
                ...
                pose_filename = self.pose_filenames[k]
                self.pose_data = np.load(os.path.join(self.data_dir, pose_filename))['arr_0']
                self.pose_std += np.sum((self.pose_data[self.train_index_map[im_filename], :] - self.pose_mean) ** 2,
                                        axis=0)
    ...
    def _compute_indices_object_wise(self):
        ...
        last_file_object_ids = \
        np.load(os.path.join(self.data_dir, self.obj_id_filenames[len(self.obj_id_filenames) - 1]))['arr_0']
    ...
    def _compute_indices_pose_wise(self):
        ...
        last_file_pose_ids = \
        np.load(os.path.join(self.data_dir, self.stable_pose_filenames[len(self.stable_pose_filenames) - 1]))['arr_0']
    ...
    def _setup_data_filenames(self):
        ...
        if self.image_mode == ImageMode.BINARY:
            ...
            self.im_filenames = [f for f in all_filenames if
                                 f.find(ImageFileTemplates.binary_im_tf_tensor_template) > -1]
        elif self.image_mode == ImageMode.COLOR_TF:
            self.im_filenames = [f for f in all_filenames if
                                 f.find(ImageFileTemplates.color_im_tf_tensor_template) > -1]
            ...
            self.im_filenames = [f for f in all_filenames if
                                 f.find(ImageFileTemplates.depth_im_tf_tensor_template) > -1]
        ...
        if len(self.im_filenames) == 0 or len(self.pose_filenames) == 0 or len(self.label_filenames) == 0 or len(
                self.stable_pose_filenames) == 0:
            ...
            while start_i < self.train_batch_size:
                ...
                if self.training_mode == TrainingMode.REGRESSION:
                    if self.preproc_mode == PreprocMode.NORMALIZATION:
                        self.train_label_arr = (self.train_label_arr - self.min_metric) / (
                        self.max_metric - self.min_metric)
                elif self.training_mode == TrainingMode.CLASSIFICATION:
                ...
                train_poses[start_i:end_i, :] = self._read_pose_data(np.copy(self.train_poses_arr),
    ...
    def _distort(self, num_loaded):
        ...
        if self.cfg['image_dropout']:
            for i in range(self.num_images):
                if np.random.rand() < self.cfg['image_dropout_rate']:
                    ...
                    x_radii = ss.gamma.rvs(self.cfg['dropout_radius_shape'], scale=self.cfg['dropout_radius_scale'],
                                           size=num_dropout_regions)
                    y_radii = ss.gamma.rvs(self.cfg['dropout_radius_shape'], scale=self.cfg['dropout_radius_scale'],
                                           size=num_dropout_regions)
                    ...
                    for j in range(num_dropout_regions):
                        ...
                        x_radius = x_radii[j]
                        y_radius = y_radii[j]
                        dropout_px_y, dropout_px_x = sd.ellipse(dropout_center[0], dropout_center[1], y_radius,
                                                                x_radius, shape=train_image.shape)
        ...
        if self.cfg['gradient_dropout']:
            for i in range(self.num_images):
                if np.random.rand() < self.cfg['gradient_dropout_rate']:
                    ...
                    thresh = ss.gamma.rvs(self.cfg['gradient_dropout_shape'], self.cfg['gradient_dropout_scale'],
                                          size=1)
        ...
        if self.cfg['border_distortion']:
            for i in range(self.num_images):
                ...
                # sample ellipses
                dropout_centers = np.random.choice(num_nonzero, size=num_dropout_regions)
                x_radii = ss.gamma.rvs(self.cfg['border_radius_shape'], scale=self.cfg['border_radius_scale'],
                                       size=num_dropout_regions)
                y_radii = ss.gamma.rvs(self.cfg['border_radius_shape'], scale=self.cfg['border_radius_scale'],
                                       size=num_dropout_regions)
                ...
                for j in range(num_dropout_regions):
                    ...
                    x_radius = x_radii[j]
                    y_radius = y_radii[j]
                    dropout_px_y, dropout_px_x = sd.ellipse(dropout_center[0], dropout_center[1], y_radius, x_radius,
        ...
        if self.cfg['background_denoising']:
            for i in range(self.num_images):
                ...
                if np.random.rand() < self.cfg['background_rate']:
                    train_image[train_image > 0] = self.cfg['background_min_depth'] + (
                                                                                      self.cfg['background_max_depth'] -
                                                                                      self.cfg[
                                                                                          'background_min_depth']) * np.random.rand()
        ...
        if self.cfg['symmetrize']:
            for i in range(self.num_images):
                ...
                if np.random.rand() < 0.5:
                    ...
                    train_image = cv2.warpAffine(train_image, rot_map, (self.im_height, self.im_width),
    ...
    def _error_rate_in_batches(self):
        ...
        for data_filename, pose_filename, label_filename in zip(self.im_filenames, self.pose_filenames,
